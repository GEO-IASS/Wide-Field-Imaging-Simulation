...
for i in range(N):
	time5 = time.time()
	clean(vis = 'sim_mwa_12h.ms',imagename='sim_image_wp_'+str(wn[i]), gridmode= 'widefield', \
	wprojplanes=wn[i],facets=1,niter = 1, imsize= [512, 512], cell= ['208.5arcsec', '208.5arcsec'],\
	weighting='natural' )
	time6 = time.time()
	delt_time3 =round((time6-time5),2)

	imview(raster = 'sim_image_wp_'+str(wn[i])+'.image',out ='wproj'+str(wn[i])+'.png')
	...
	pixelSum = 0.0
		...
		if (margin_ra_h-float(j[0])<0.04) or (margin_dec_h-float(j[1])<0.04):
			continue
		xval = imval('sim_image_wp_'+str(wn[i])+'.image',region='circle[['+j[0]+'deg,'+j[1]+'deg'+'],1.0deg]')
...
wide_field_tech = []
for i in range(N):
	wide_field_tech.append('wp_'+str(wn[i]))
