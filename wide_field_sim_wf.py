...
N = int(plist[0])
print 'the number of wp simulation is: %s'%N
...
i = 0
while(i<len(plist)):
	fn.append(int(plist[i]))
	wn.append(int(plist[i+1]))
	i+=2
...
for i in range(N):
	time7 = time.time()
	clean(vis = 'sim_mwa_12h.ms',imagename='sim_image_fc+wp_f'+str(fn[i])+'w'+str(wn[i]),gridmode= 'widefield', \
	wprojplanes=wn[i],facets=fn[i],niter = 1, imsize= [512, 512], cell= ['208.5arcsec', '208.5arcsec'],\
	weighting='natural' )
	time8 = time.time()
	delt_time4 =round((time8-time7),2)

	imview(raster = 'sim_image_fc+wp_f'+str(fn[i])+'w'+str(wn[i])+'.image', out ='fc'+str(fn[i])+'wp'+str(wn[i])+'.png')
	...
	pixelSum = 0.0
		...
		if (margin_ra_h-float(j[0])<0.04) or (margin_dec_h-float(j[1])<0.04):
			continue
		xval = imval('sim_image_fc+wp_f'+str(fn[i])+'w'+str(wn[i])+'.image',region='circle[['+j[0]+'deg,'+j[1]+'deg'+'],1.0deg]')
